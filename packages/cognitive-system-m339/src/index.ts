import {BehaviorPredictionState, BehaviorPredictionMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m339_${Date.now()}`;
...
export class BehaviorPrediction {
  ...
  getState(): BehaviorPredictionState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): BehaviorPredictionMetrics {
    return {
