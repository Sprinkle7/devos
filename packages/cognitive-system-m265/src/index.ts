import {ClassificationLearningState, ClassificationLearningMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m265_${Date.now()}`;
...
export class ClassificationLearning {
  ...
  getState(): ClassificationLearningState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): ClassificationLearningMetrics {
    return {
