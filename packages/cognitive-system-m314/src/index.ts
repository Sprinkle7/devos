import {WorkloadBalancingState, WorkloadBalancingMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m314_${Date.now()}`;
...
export class WorkloadBalancing {
  ...
  getState(): WorkloadBalancingState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): WorkloadBalancingMetrics {
    return {
