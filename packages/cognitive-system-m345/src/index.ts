import {AnomalyClassificationState, AnomalyClassificationMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m345_${Date.now()}`;
...
export class AnomalyClassification {
  ...
  getState(): AnomalyClassificationState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): AnomalyClassificationMetrics {
    return {
