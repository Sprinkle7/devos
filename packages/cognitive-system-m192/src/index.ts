import {CuriosityDriveState, CuriosityDriveMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m192_${Date.now()}`;
...
export class CuriosityDrive {
  ...
  getMetrics(): CuriosityDriveMetrics {
    return {
