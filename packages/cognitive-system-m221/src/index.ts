import {SimilarityAssessmentState, SimilarityAssessmentMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m221_${Date.now()}`;
...
export class SimilarityAssessment {
  ...
  getState(): SimilarityAssessmentState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): SimilarityAssessmentMetrics {
    return {
