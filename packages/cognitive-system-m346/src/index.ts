import {AlertGenerationState, AlertGenerationMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m346_${Date.now()}`;
...
export class AlertGeneration {
  ...
  getMetrics(): AlertGenerationMetrics {
    return {
