import {LatencyMeasurementState, LatencyMeasurementMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m286_${Date.now()}`;
...
export class LatencyMeasurement {
  ...
  getState(): LatencyMeasurementState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): LatencyMeasurementMetrics {
    return {
