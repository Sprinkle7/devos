import {GreedySelectionState, GreedySelectionMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m251_${Date.now()}`;
...
export class GreedySelection {
  ...
  getMetrics(): GreedySelectionMetrics {
    return {
