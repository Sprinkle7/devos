import {DepthFirstSearchState, DepthFirstSearchMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m258_${Date.now()}`;
...
export class DepthFirstSearch {
  ...
  getState(): DepthFirstSearchState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): DepthFirstSearchMetrics {
    return {
