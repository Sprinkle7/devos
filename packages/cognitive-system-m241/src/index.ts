import {ValueAggregationState, ValueAggregationMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m241_${Date.now()}`;
...
export class ValueAggregation {
  ...
  getState(): ValueAggregationState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): ValueAggregationMetrics {
    return {
