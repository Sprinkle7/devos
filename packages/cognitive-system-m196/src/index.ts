import {ApproachAvoidanceState, ApproachAvoidanceMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m196_${Date.now()}`;
...
export class ApproachAvoidance {
  ...
  getState(): ApproachAvoidanceState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): ApproachAvoidanceMetrics {
    return {
