import {ConcretizationState, ConcretizationMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m220_${Date.now()}`;
...
export class Concretization {
  ...
  getMetrics(): ConcretizationMetrics {
    return {
