import {ThroughputAnalysisState, ThroughputAnalysisMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m285_${Date.now()}`;
...
export class ThroughputAnalysis {
  ...
  getState(): ThroughputAnalysisState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): ThroughputAnalysisMetrics {
    return {
