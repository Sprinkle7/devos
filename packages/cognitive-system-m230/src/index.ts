import {SequentialProcessingState, SequentialProcessingMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m230_${Date.now()}`;
...
export class SequentialProcessing {
  ...
  getState(): SequentialProcessingState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): SequentialProcessingMetrics {
    return {
