import {AmbiguityToleranceState, AmbiguityToleranceMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m188_${Date.now()}`;
...
export class AmbiguityTolerance {
  ...
  getState(): AmbiguityToleranceState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): AmbiguityToleranceMetrics {
    return {
