import {StateSpaceExplorationState, StateSpaceExplorationMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m248_${Date.now()}`;
...
export class StateSpaceExploration {
  ...
  getState(): StateSpaceExplorationState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): StateSpaceExplorationMetrics {
    return {
