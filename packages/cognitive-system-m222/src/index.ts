import {DifferenceDetectionState, DifferenceDetectionMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m222_${Date.now()}`;
...
export class DifferenceDetection {
  ...
  getState(): DifferenceDetectionState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): DifferenceDetectionMetrics {
    return {
