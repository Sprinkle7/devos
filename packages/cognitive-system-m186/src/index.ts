import {ImpulseControlState, ImpulseControlMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m186_${Date.now()}`;
...
export class ImpulseControl {
  ...
  getMetrics(): ImpulseControlMetrics {
    return {
