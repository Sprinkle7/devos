import {ExplorationMotivationState, ExplorationMotivationMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m193_${Date.now()}`;
...
export class ExplorationMotivation {
  ...
  getState(): ExplorationMotivationState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): ExplorationMotivationMetrics {
    return {
