import {BehavioralInhibitionState, BehavioralInhibitionMetrics} from './types';

class Subsystem {
  private data: Map<string, any> = new Map();
  registerItem(value: number): any {
    const id = `m195_${Date.now()}`;
...
export class BehavioralInhibition {
  ...
  getState(): BehavioralInhibitionState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): BehavioralInhibitionMetrics {
    return {
