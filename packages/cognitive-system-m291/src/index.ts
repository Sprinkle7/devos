import {BufferManagementState, BufferManagementMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m291_${Date.now()}`;
...
export class BufferManagement {
  ...
  getState(): BufferManagementState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): BufferManagementMetrics {
    return {
