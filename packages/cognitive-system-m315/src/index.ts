import {PriorityQueuingState, PriorityQueuingMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m315_${Date.now()}`;
...
export class PriorityQueuing {
  ...
  getMetrics(): PriorityQueuingMetrics {
    return {
