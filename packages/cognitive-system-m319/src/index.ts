import {CongestionManagementState, CongestionManagementMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m319_${Date.now()}`;
...
export class CongestionManagement {
  ...
  getState(): CongestionManagementState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): CongestionManagementMetrics {
    return {
