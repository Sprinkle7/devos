import {SituationAssessmentState, SituationAssessmentMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m341_${Date.now()}`;
...
export class SituationAssessment {
  ...
  getState(): SituationAssessmentState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): SituationAssessmentMetrics {
    return {
