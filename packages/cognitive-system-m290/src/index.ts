import {GarbageCollectionState, GarbageCollectionMetrics} from './types';

class CoreSubsystem {
  private data: Map<string, any> = new Map();
  register(value: number): any {
    const id = `m290_${Date.now()}`;
...
export class GarbageCollection {
  ...
  getState(): GarbageCollectionState {
    this.updateMetrics();
    return {
  ...
  getMetrics(): GarbageCollectionMetrics {
    return {
